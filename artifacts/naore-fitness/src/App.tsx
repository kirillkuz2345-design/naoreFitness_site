...
const blogPosts: {
...
}[] = [
  {
    ...
    title: 'Как вести клиентов фитнес-тренеру: Excel, мессенджеры или единая система',
    description: 'Excel, мессенджеры или единая система для ведения клиентов тренера — сравниваем подходы и разбираем, когда пора переходить на единое рабочее пространство.',
    h1: 'Как вести клиентов фитнес-тренеру: Excel, мессенджеры или единая система',
    lead: 'Excel, заметки и переписка работают, пока клиентов немного. С ростом практики информация расползается, историю сложно искать, а контекст теряется. Разберём три подхода и момент, когда пора менять инструмент.',
    sections: [
      ['Excel и таблицы', 'Таблица кажется бесплатной и гибкой, но быстро превращается в десяток вкладок, которые ведёшь вручную. Данные дублируются, а программу и прогресс клиента приходится держать в разных файлах.'],
      ['Мессенджеры и заметки', 'В WhatsApp и Telegram удобно быстро списаться, но там нет структуры: планы, правки и вопросы тонут в ленте, а история по конкретному клиенту не собирается в одном месте.'],
  ...
  {
    slug: 'kak-uyti-ot-excel', cluster: 'Excel / CRM',
    title: 'Как уйти от Excel в работе тренера',
    description: 'Как фитнес-тренеру перейти с Excel на единую систему без боли: что переносить в первую очередь и как связать данные.',
    h1: 'Как уйти от Excel в работе тренера',
    lead: 'Excel гибкий, но с ростом практики становится источником ошибок и потерянного времени. Разберём, как перейти на систему без боли.',
    sections: [
      ['Почему Excel перестаёт справляться', 'Вкладки растут, формулы ломаются, данные дублируются, а связей между клиентом, программой и прогрессом нет.'],
    ...
    cta: { title: 'Замените Excel единой системой', text: 'CRM NAORE связывает клиентов, тренировки, результаты и общение.', label: 'CRM для тренера', href: '/crm' },
  ...
  },
  {
    ...
    title: 'Excel или CRM для персонального тренера',
    description: 'Excel или CRM для персонального тренера: сравниваем по стоимости, структуре данных, масштабу и истории — что выбрать.',
    h1: 'Excel или CRM для персонального тренера',
    lead: 'Excel бесплатный и знакомый, CRM — системная и связанная. Сравним по задачам персонального тренера и решим, что выбрать.',
    sections: [
      ['Стоимость и порог входа', 'Excel не требует освоения, но «бесплатность» съедается ручным трудом. CRM требует старта, но экономит время дальше.'],
      ['Структура данных', 'В Excel связи между клиентом, программой и прогрессом делаются руками. В CRM они встроены.'],
      ['Масштаб', 'Одна-две вкладки работают для пары клиентов; на десятках таблица становится тормозом, а CRM — наоборот.'],
      ['История и поиск', 'В CRM история клиента собирается автоматически и легко ищется; в Excel и чатах — теряется.'],
      ['Что выбрать', 'Пара клиентов и простые задачи — Excel допустим. Растущая практика — CRM окупается временем и порядком.'],
    ],
    cta: { title: 'Перейдите с Excel на CRM', text: 'NAORE заменяет таблицы связанной системой для тренера.', label: 'CRM для тренера', href: '/crm' },
    related: [['Как уйти от Excel', '/blog/kak-uyti-ot-excel'], ['Ведение клиентов', '/clients'], ['Автоматизация', '/automation']],
  },
  {
  ...
  },
  {
    ...
    related: [['Excel или CRM для персонального тренера', '/blog/excel-ili-crm'], ['Ведение клиентов', '/clients'], ['Автоматизация', '/automation']],
  },
];
