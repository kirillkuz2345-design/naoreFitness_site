...
function Home() {
  ...
  return <div className="site-shell noise"><a href="#main" className="skip-link">Перейти к содержанию</a><Header /><main id="main">
    ...
    <section className="section band" id="pricing"><div className="container-wide"><div className="section-head" data-reveal><span className="eyebrow">Тарифы</span><h2 className="section-title">Сейчас всё бесплатно</h2><p className="section-copy">Вся функциональность NAORE доступна бесплатно — и тренерам, и атлетам. Платные тарифы появятся позже, а ранние пользователи получат лучшие условия.</p></div><div className="free-card" data-reveal><span className="free-badge"><span className="pulse-dot" />Ранний доступ · всё включено</span><div className="free-list"><div className="free-item"><span className="value-check"><Check size={13} /></span> <span>Конструктор тренировок</span></div><div className="free-item"><span className="value-check"><Check size={13} /></span> <span>Ведение клиентов и CRM</span></div><div className="free-item"><span className="value-check"><Check size={13} /></span> <span>Аналитика прогресса</span></div><div className="free-item"><span className="value-check"><Check size={13} /></span> <span>Живой чат тренер↔клиент</span></div><div className="free-item"><span className="value-check"><Check size={13} /></span> <span>Календарь и платежи</span></div><div className="free-item"><span className="value-check"><Check size={13} /></span> <span>Калькулятор КБЖУ и офлайн (PWA)</span></div></div><div className="cta-actions"><a href={REGISTER_TRAINER} className="btn btn-primary js-magnetic" data-testid="button-free-trainer">Начать бесплатно <ArrowRight size={16} /></a><a href={REGISTER_CLIENT} className="btn btn-ghost" data-testid="button-free-client">Я атлет — тренироваться</a></div></div></div></section>
...
const productPages: Record<string, {
  ...
  clients: {
    ...
    h1: 'Ведение клиентов без Google-таблиц, чатов Telegram и бумажных тетрадок',
  ...
  crm: {
    ...
    intro: 'Клиенты, программы, результаты и общение — связанными между собой, а не разбросанными по Google-таблицам, чатам Telegram и бумажным тетрадкам.',
    ...
    sections: [
      ...
      ['Почему CRM лучше Google-таблиц, чатов Telegram и тетрадок', 'Данные связаны и не дублируются, историю легко искать, ничего не теряется в переписке.'],
    ],
  ...
  personalTrainer: {
    ...
    intro: 'Клиенты, персональные программы, прогресс и общение — в одном сервисе, без Google-таблиц, чатов Telegram и бумажных тетрадок.',
    ...
    sections: [
      ...
      ['Работайте без Google-таблиц, чатов Telegram и тетрадок', 'Один рабочий центр вместо разрозненных инструментов.'],
    ],
  ...
  automation: {
    ...
    intro: 'Меньше ручного переноса данных между Google-таблицами, чатами Telegram и бумажными тетрадками — больше времени на реальную работу с клиентами.',
...
// ===== Блог (контент-план, первая волна) =====
const blogPosts: {
  slug: string; cluster: string; title: string; description: string; h1: string; lead: string;
  sections: [string, string][]; cta: { title: string; text: string; label: string; href: string }; related: [string, string][];
}[] = [
  {
    slug: 'kak-vesti-klientov', cluster: 'Ведение клиентов',
    title: 'Как вести клиентов фитнес-тренеру: Google-таблицы, чаты Telegram или единая система',
    description: 'Google-таблицы, чаты Telegram или единая система для ведения клиентов тренера — сравниваем подходы и разбираем, когда пора переходить на единое рабочее пространство.',
    h1: 'Как вести клиентов фитнес-тренеру: Google-таблицы, чаты Telegram или единая система',
    lead: 'Google-таблицы, бумажные тетрадки и переписка в Telegram работают, пока клиентов немного. С ростом практики информация расползается, историю сложно искать, а контекст теряется. Разберём три подхода и момент, когда пора менять инструмент.',
    sections: [
      ['Google-таблицы', 'Таблица кажется бесплатной и гибкой, но быстро превращается в десяток вкладок и файлов, которые ведёшь вручную. Данные дублируются, а программу и прогресс клиента приходится держать отдельно.'],
      ['Чаты Telegram и бумажные тетрадки', 'В чатах Telegram и тетрадках удобно быстро записать, но там нет структуры: планы, правки и вопросы тонут в ленте, а история по конкретному клиенту не собирается в одном месте.'],
      ['Единая система ведения клиентов', 'Единое рабочее пространство хранит клиентов, программы, результаты и общение связанными между собой. Открываете карточку — и сразу видите цель, план, прогресс и переписку, без переключений между приложениями.'],
      ['Когда пора переходить', 'Если вы тратите время на перенос данных между файлами и чатами, забываете детали по клиентам и не можете быстро показать динамику — таблиц и мессенджеров уже мало.'],
      ['Что важно при выборе', 'Ищите решение, где тренировки лежат рядом с данными клиента, прогресс считается автоматически, а общение не теряет контекст. Это экономит часы каждую неделю.'],
    ],
    cta: { title: 'Ведите клиентов в одном месте', text: 'NAORE собирает клиентов, тренировки, результаты и общение в едином рабочем пространстве.', label: 'Ведение клиентов', href: '/clients' },
    related: [['CRM для тренера', '/crm'], ['Конструктор тренировок', '/constructor'], ['Аналитика прогресса', '/analytics']],
  },
  {
    slug: 'avtomatizaciya-raboty-trenera', cluster: 'Автоматизация',
    title: 'Как автоматизировать работу фитнес-тренера',
    description: 'Что реально автоматизировать в работе фитнес-тренера: клиенты, программы, прогресс и коммуникация — и сколько времени это освобождает.',
    h1: 'Как автоматизировать работу фитнес-тренера',
    lead: 'Рутина съедает время, которое можно потратить на клиентов: перенос данных, одинаковые программы, напоминания и поиск истории. Разберём, что реально автоматизировать.',
    sections: [
      ['Что отнимает время', 'Больше всего часов уходит на ручные операции: переписывание планов, дублирование данных между файлами и чатами, повторную сборку одинаковых программ.'],
      ['Клиенты и их данные', 'Единая база клиентов обновляется по ходу работы: статусы, цели и история собираются сами, отдельную таблицу вести не нужно.'],
      ['Программы тренировок', 'Сохраняйте программы как шаблоны и переиспользуйте их для новых клиентов вместо сборки с нуля каждый раз.'],
      ['Прогресс и аналитика', 'Результаты выполненных тренировок собираются автоматически и превращаются в динамику — прогресс не нужно считать руками.'],
      ['Коммуникация', 'Когда общение привязано к клиенту и упражнениям, отпадает часть повторяющихся вопросов, а контекст всегда под рукой.'],
    ],
    cta: { title: 'Автоматизируйте рутину', text: 'NAORE берёт на себя перенос данных, шаблоны программ и подсчёт прогресса.', label: 'Автоматизация', href: '/automation' },
    related: [['CRM для тренера', '/crm'], ['Ведение клиентов', '/clients'], ['Конструктор тренировок', '/constructor']],
  },
  {
    slug: 'kak-sostavit-programmu-trenirovok', cluster: 'Тренировки',
    title: 'Как составить программу тренировок для клиента',
    description: 'Пошагово: как составить персональную программу тренировок под цель и уровень клиента и как ускорить сборку.',
    h1: 'Как составить программу тренировок для клиента',
    lead: 'Хорошая программа учитывает цель, уровень и восстановление клиента, а не собирается «на глаз». Разберём шаги и как ускорить сборку.',
    sections: [
      ['Определите цель и уровень', 'Начните с цели (сила, масса, снижение веса, здоровье) и текущего уровня клиента — от этого зависят выбор упражнений, объём и интенсивность.'],
      ['Подберите упражнения', 'Соберите базовые и вспомогательные упражнения под цель. Библиотека с параметрами ускоряет выбор и держит формулировки одинаковыми.'],
      ['Задайте подходы, повторы и прогрессию', 'Определите подходы, повторы, вес и отдых, а также как нагрузка будет расти неделя за неделей — прогрессивная нагрузка даёт результат.'],
      ['Спланируйте неделю', 'Распределите тренировки по дням с учётом восстановления. Расписание помогает клиенту держать ритм и не пропускать.'],
      ['Отслеживайте и корректируйте', 'После старта смотрите выполнение и результаты и корректируйте план по данным, а не по ощущениям.'],
    ],
    cta: { title: 'Соберите программу за минуты', text: 'Конструктор NAORE помогает собрать персональную программу и назначить её клиенту.', label: 'Конструктор тренировок', href: '/constructor' },
    related: [['Аналитика прогресса', '/analytics'], ['Ведение клиентов', '/clients'], ['Отслеживание прогресса', '/progress']],
  },
  {
    slug: 'kak-otslezhivat-progress-klienta', cluster: 'Аналитика',
    title: 'Как отслеживать прогресс клиента',
    description: 'Что и как отслеживать в прогрессе клиента, чтобы вовремя корректировать программу и вести к результату.',
    h1: 'Как отслеживать прогресс клиента',
    lead: 'Без цифр прогресс оценивается «на глаз», и корректировки запаздывают. Разберём, что и как отслеживать, чтобы вести клиента к цели.',
    sections: [
      ['Фиксируйте результаты каждой тренировки', 'История выполненных тренировок с весами и повторами — основа для любых выводов. Без записей динамику не построить.'],
      ['Смотрите динамику по неделям', 'Тренды по объёму и результатам показывают, работает ли план, раньше, чем это заметно по ощущениям.'],
      ['Контролируйте выполнение программы', 'Отдельно смотрите, какие тренировки выполнены, а какие пропущены — дисциплина клиента напрямую влияет на результат.'],
      ['Анализируйте прогресс по упражнениям', 'Рост по ключевым упражнениям (вес, объём, повторы) показывает, где прогресс идёт, а где остановился.'],
      ['Принимайте решения по данным', 'Меняйте нагрузку там, где показатели встали, и закрепляйте то, что работает.'],
    ],
    cta: { title: 'Видьте прогресс, а не список тренировок', text: 'Аналитика NAORE превращает результаты клиента в понятную динамику.', label: 'Аналитика прогресса', href: '/analytics' },
    related: [['Отслеживание прогресса', '/progress'], ['Конструктор тренировок', '/constructor'], ['Ведение клиентов', '/clients']],
  },
  {
    slug: 'kak-vesti-mnogo-klientov', cluster: 'Ведение клиентов',
    title: 'Как вести много клиентов одновременно',
    description: 'Как фитнес-тренеру вести много клиентов одновременно без потери качества и выгорания: стандартизация, карточки клиентов и шаблоны.',
    h1: 'Как вести много клиентов одновременно',
    lead: 'С десятком клиентов держать всё в голове уже нельзя. Разберём, как вести много клиентов без потери качества и выгорания.',
    sections: [
      ['Стандартизируйте процесс', 'Единый порядок работы с каждым клиентом (онбординг, программа, контроль) снимает лишние решения и ускоряет ведение.'],
      ['Держите всё в карточке клиента', 'Когда цель, план, прогресс и переписка лежат в одной карточке, вы за секунды вспоминаете контекст любого клиента.'],
      ['Используйте шаблоны программ', 'Похожие цели — похожие программы. Шаблоны экономят время, а персонализация остаётся точечной.'],
      ['Следите за выполнением сразу по всем', 'Общий обзор показывает, кто отстаёт и кому нужна корректировка, без ручной проверки каждого.'],
      ['Не смешивайте личное и рабочее', 'Отдельное рабочее пространство вместо личных мессенджеров снимает хаос и помогает не выгорать.'],
    ],
    cta: { title: 'Ведите десятки клиентов без хаоса', text: 'NAORE держит всех клиентов, программы и прогресс в одном месте.', label: 'Ведение клиентов', href: '/clients' },
    related: [['CRM для тренера', '/crm'], ['Автоматизация', '/automation'], ['Аналитика прогресса', '/analytics']],
  },
  {
    slug: 'kak-uyti-ot-excel', cluster: 'Таблицы / CRM',
    title: 'Как уйти от Google-таблиц, чатов Telegram и тетрадок в работе тренера',
    description: 'Как фитнес-тренеру перейти с Google-таблиц, чатов Telegram и тетрадок на единую систему без боли: что переносить в первую очередь и как связать данные.',
    h1: 'Как уйти от Google-таблиц, чатов Telegram и тетрадок в работе тренера',
    lead: 'Google-таблицы гибкие, но с ростом практики становятся источником ошибок и потерянного времени. Разберём, как перейти на систему без боли.',
    sections: [
      ['Почему Google-таблицы и чаты перестают справляться', 'Вкладки растут, формулы ломаются, данные дублируются, а связей между клиентом, программой и прогрессом нет.'],
      ['Что перенести в первую очередь', 'Начните с базы клиентов и их целей, затем программы и историю тренировок — так вы сразу получите единый контекст.'],
      ['Свяжите данные между собой', 'В системе клиент, программа, результаты и общение связаны. Не нужно сопоставлять строки из разных файлов.'],
      ['Сохраните привычную гибкость', 'Хорошая система оставляет свободу настройки, но убирает ручной труд и дублирование.'],
      ['Переходите постепенно', 'Переносите клиентов волнами, а не всех сразу — так процесс не остановится.'],
    ],
    cta: { title: 'Замените таблицы и чаты единой системой', text: 'CRM NAORE связывает клиентов, тренировки, результаты и общение.', label: 'CRM для тренера', href: '/crm' },
    related: [['Ведение клиентов', '/clients'], ['Автоматизация', '/automation'], ['Конструктор тренировок', '/constructor']],
  },
  {
    slug: 'crm-dlya-fitnes-trenera', cluster: 'CRM',
    title: 'CRM для фитнес-тренера: зачем нужна и что автоматизирует',
    description: 'Зачем фитнес-тренеру CRM и что она автоматизирует: клиенты, программы, результаты и коммуникация в одном рабочем центре.',
    h1: 'CRM для фитнес-тренера: зачем нужна и что автоматизирует',
    lead: 'CRM для тренера — не «программа для продаж», а рабочий центр, где живут клиенты, программы и результаты. Разберём, зачем она и что автоматизирует.',
    sections: [
      ['Что такое CRM для тренера', 'Система, где хранятся клиенты, программы, результаты и общение — связанными между собой, а не по отдельным файлам.'],
      ['Что она автоматизирует', 'Обновление данных о клиентах, шаблоны программ, сбор результатов и подсчёт прогресса — то, что раньше делалось вручную.'],
      ['Единый контекст клиента', 'Открываете карточку и видите цель, план, динамику и переписку — без переключений между приложениями.'],
      ['Меньше потерь и ошибок', 'Данные не дублируются и не теряются в чатах, а историю легко искать.'],
      ['Когда CRM оправдана', 'Если клиентов больше нескольких и вы тратите время на рутину — CRM возвращает часы в неделю.'],
    ],
    cta: { title: 'Попробуйте CRM для тренера', text: 'NAORE — CRM, заточенная под работу фитнес-тренера с клиентами.', label: 'CRM для тренера', href: '/crm' },
    related: [['Ведение клиентов', '/clients'], ['Автоматизация', '/automation'], ['Аналитика прогресса', '/analytics']],
  },
  {
    slug: 'excel-ili-crm', cluster: 'Сравнения',
    title: 'Google-таблицы или CRM для персонального тренера',
    description: 'Google-таблицы или CRM для персонального тренера: сравниваем по стоимости, структуре данных, масштабу и истории — что выбрать.',
    h1: 'Google-таблицы или CRM для персонального тренера',
    lead: 'Google-таблицы бесплатны и знакомы, CRM — системная и связанная. Сравним по задачам персонального тренера и решим, что выбрать.',
    sections: [
      ['Стоимость и порог входа', 'Google-таблицы не требуют освоения, но «бесплатность» съедается ручным трудом. CRM требует старта, но экономит время дальше.'],
      ['Структура данных', 'В Google-таблицах связи между клиентом, программой и прогрессом делаются руками. В CRM они встроены.'],
      ['Масштаб', 'Одна-две вкладки работают для пары клиентов; на десятках таблица становится тормозом, а CRM — наоборот.'],
      ['История и поиск', 'В CRM история клиента собирается автоматически и легко ищется; в Google-таблицах и чатах Telegram — теряется.'],
      ['Что выбрать', 'Пара клиентов и простые задачи — Google-таблицы допустимы. Растущая практика — CRM окупается временем и порядком.'],
    ],
    cta: { title: 'Перейдите с таблиц на CRM', text: 'NAORE заменяет таблицы связанной системой для тренера.', label: 'CRM для тренера', href: '/crm' },
    related: [['Как уйти от таблиц и чатов', '/blog/kak-uyti-ot-excel'], ['Ведение клиентов', '/clients'], ['Автоматизация', '/automation']],
  },
  {
    slug: 'rabota-onlayn-trenera', cluster: 'Онлайн-тренер',
    title: 'Как организовать работу онлайн-тренера',
    description: 'Как организовать работу онлайн-тренера: собрать клиентов, назначать программы дистанционно, контролировать выполнение и масштабироваться.',
    h1: 'Как организовать работу онлайн-тренера',
    lead: 'Онлайн-формат снимает границы, но добавляет хаоса: клиенты в разных чатах, планы в файлах, прогресс — на словах. Разберём, как выстроить систему.',
    sections: [
      ['Соберите клиентов в одном месте', 'Единая база вместо десятка переписок: по каждому клиенту видно цель, план и прогресс.'],
      ['Назначайте программы дистанционно', 'Клиент получает актуальный план в приложении — без пересланных файлов и путаницы с версиями.'],
      ['Контролируйте выполнение', 'Отметки о выполнении показывают, кто держит план, без напоминаний в мессенджерах.'],
      ['Опирайтесь на аналитику', 'Динамика результатов помогает вести клиента к цели по данным, а не по ощущениям.'],
      ['Масштабируйтесь без потери качества', 'Шаблоны и единый процесс позволяют брать больше клиентов, сохраняя персональный подход.'],
    ],
    cta: { title: 'Организуйте онлайн-практику', text: 'NAORE даёт онлайн-тренеру клиентов, программы, прогресс и общение в одном месте.', label: 'Онлайн-тренер', href: '/online-trainer' },
    related: [['Ведение клиентов', '/clients'], ['Коммуникация', '/communication'], ['Автоматизация', '/automation']],
  },
  {
    slug: 'informaciya-o-klientah-v-messendzherah', cluster: 'Мессенджеры',
    title: 'Как не потерять информацию о клиентах в WhatsApp и Telegram',
    description: 'Почему в WhatsApp и Telegram теряется контекст клиента и как перестать терять важное: договорённости, изменения программы и результаты.',
    h1: 'Как не потерять информацию о клиентах в WhatsApp и Telegram',
    lead: 'В мессенджерах теряется главное — контекст клиента. Разберём, почему так и как перестать терять важное.',
    sections: [
      ['Почему мессенджеры теряют контекст', 'Планы, правки и вопросы идут вперемешку с личным, а история по клиенту не собирается в одном месте.'],
      ['Что теряется чаще всего', 'Договорённости, изменения программы и результаты — то, что потом сложно восстановить по ленте.'],
      ['Привяжите общение к клиенту', 'Когда сообщения связаны с карточкой клиента и упражнениями, контекст не рассыпается.'],
      ['Держите тренировки рядом с перепиской', 'Ответ на вопрос по упражнению — рядом с самим упражнением, а не в отдельном чате.'],
      ['Единое пространство вместо чатов', 'Одно место для общения и тренировок убирает переключения и потери.'],
    ],
    cta: { title: 'Не теряйте контекст клиента', text: 'В NAORE общение живёт рядом с тренировками и прогрессом.', label: 'Коммуникация', href: '/communication' },
    related: [['Ведение клиентов', '/clients'], ['CRM для тренера', '/crm'], ['Конструктор тренировок', '/constructor']],
  },
  {
    slug: 'chem-zamenit-whatsapp', cluster: 'Мессенджеры',
    title: 'Чем заменить WhatsApp для работы тренера',
    description: 'Чем заменить WhatsApp в работе фитнес-тренера: рабочее пространство, где общение связано с клиентом, тренировками и прогрессом.',
    h1: 'Чем заменить WhatsApp для работы тренера',
    lead: 'WhatsApp удобен для быстрых сообщений, но как рабочий инструмент тренера проигрывает. Разберём, чем его заменить.',
    sections: [
      ['Проблема WhatsApp для работы', 'Нет структуры, история теряется, личное смешано с рабочим, а планы и результаты живут отдельно от переписки.'],
      ['Что должно быть у замены', 'Общение рядом с тренировками и прогрессом, контекст по каждому клиенту и понятная история.'],
      ['Не просто чат, а рабочее пространство', 'Замена — это система, где переписка связана с клиентом и его планом, а не отдельный мессенджер.'],
      ['Меньше повторяющихся вопросов', 'Когда клиент видит план и прогресс сам, часть вопросов отпадает.'],
      ['Как перейти', 'Переносите общение с клиентами в единое пространство постепенно, начиная с активных.'],
    ],
    cta: { title: 'Замените мессенджеры рабочим пространством', text: 'NAORE соединяет общение, тренировки и прогресс в одном месте.', label: 'Коммуникация', href: '/communication' },
    related: [['Как не потерять информацию о клиентах', '/blog/informaciya-o-klientah-v-messendzherah'], ['Ведение клиентов', '/clients'], ['CRM для тренера', '/crm']],
  },
  {
    slug: 'dnevnik-trenirovok', cluster: 'Дневник',
    title: 'Дневник тренировок: что должен видеть тренер и клиент',
    description: 'Что должен показывать дневник тренировок тренеру и клиенту: программу, выполнение, результаты, историю и связь с прогрессом.',
    h1: 'Дневник тренировок: что должен видеть тренер и клиент',
    lead: 'Дневник тренировок — не просто список. Разберём, что важно видеть тренеру и клиенту, чтобы он приносил пользу.',
    sections: [
      ['Что видит клиент', 'Свою программу, отметки о выполнении, результаты и динамику — понятно, движется ли он к цели.'],
      ['Что видит тренер', 'Те же данные плюс историю: что выполнено, где прогресс, а где нужна корректировка.'],
      ['История и результаты', 'Полный архив тренировок с весами и повторами — основа для аналитики и решений.'],
      ['Связь с прогрессом', 'Записи автоматически складываются в динамику, а не остаются набором строк.'],
      ['Доступ офлайн', 'Дневник под рукой даже без интернета (PWA) — тренировку легко отметить прямо в зале.'],
    ],
    cta: { title: 'Ведите дневник тренировок онлайн', text: 'NAORE хранит историю тренировок и превращает её в прогресс.', label: 'Дневник тренировок', href: '/workout-diary' },
    related: [['Отслеживание прогресса', '/progress'], ['Аналитика прогресса', '/analytics'], ['Конструктор тренировок', '/constructor']],
  },
  {
    slug: 'kak-analizirovat-progress', cluster: 'Аналитика',
    title: 'Как анализировать прогресс клиента',
    description: 'Как анализировать прогресс клиента: смотреть тренды, разделять объём и интенсивность, сравнивать периоды и превращать выводы в корректировки.',
    h1: 'Как анализировать прогресс клиента',
    lead: 'Анализ прогресса — это не просто «стало лучше или хуже». Разберём, на что смотреть, чтобы принимать решения по тренировкам.',
    sections: [
      ['Соберите данные', 'Без истории тренировок и результатов анализировать нечего — сначала фиксируйте каждую тренировку.'],
      ['Смотрите тренды, а не отдельные дни', 'Один результат мало что говорит; важна динамика за недели.'],
      ['Разделяйте объём, интенсивность и выполнение', 'Рост объёма, прибавка в весах и дисциплина выполнения — разные сигналы, их стоит смотреть отдельно.'],
      ['Сравнивайте периоды', 'Сравнение недель и месяцев показывает, работает ли текущий подход.'],
      ['Превращайте анализ в действия', 'Вывод без изменения плана бесполезен — корректируйте нагрузку по данным.'],
    ],
    cta: { title: 'Анализируйте прогресс по данным', text: 'Аналитика NAORE показывает динамику клиента и помогает корректировать план.', label: 'Аналитика прогресса', href: '/analytics' },
    related: [['Как отслеживать прогресс клиента', '/blog/kak-otslezhivat-progress-klienta'], ['Отслеживание прогресса', '/progress'], ['Конструктор тренировок', '/constructor']],
  },
  {
    slug: 'luchshie-programmy-dlya-trenerov', cluster: 'Сравнения',
    title: 'Лучшие программы для фитнес-тренеров',
    description: 'Как выбрать программу для фитнес-тренера: на что смотреть — клиенты, тренировки, аналитика прогресса и коммуникация в одном месте.',
    h1: 'Лучшие программы для фитнес-тренеров: на что смотреть при выборе',
    lead: '«Лучшая» программа — та, что закрывает задачи именно тренера: клиенты, тренировки, прогресс и общение. Разберём, на что смотреть при выборе.',
    sections: [
      ['Начните с задач, а не с функций', 'Сначала определите, что болит: хаос в клиентах, сборка программ, контроль прогресса или общение.'],
      ['Клиенты и программы в одном месте', 'Хорошая программа связывает клиента, его план и результаты, а не хранит их отдельно.'],
      ['Аналитика прогресса', 'Динамика результатов отличает рабочий инструмент от простого списка тренировок.'],
      ['Коммуникация рядом с тренировками', 'Общение с клиентом должно жить рядом с планом, а не в отдельном мессенджере.'],
      ['Простота и русскоязычность', 'Инструмент должен быть понятным и на вашем языке, иначе им не будут пользоваться.'],
    ],
    cta: { title: 'Рабочее пространство персонального тренера', text: 'NAORE закрывает задачи тренера: клиенты, программы, прогресс и общение.', label: 'Персональный тренер', href: '/personal-trainer' },
    related: [['CRM для тренера', '/crm'], ['Ведение клиентов', '/clients'], ['Конструктор тренировок', '/constructor']],
  },
  {
    slug: 'luchshie-crm-dlya-trenerov', cluster: 'Сравнения',
    title: 'Лучшие CRM для фитнес-тренеров',
    description: 'Как выбрать CRM для фитнес-тренера: заточенность под тренера, связь данных, аналитика прогресса, коммуникация и стоимость.',
    h1: 'Лучшие CRM для фитнес-тренеров: критерии выбора',
    lead: 'CRM для тренера отличается от «продажной»: в центре — клиенты, тренировки и прогресс. Разберём, по каким критериям выбирать.',
    sections: [
      ['Заточенность под тренера', 'Общая CRM про сделки; тренеру нужна система про клиентов, программы и результаты.'],
      ['Связь данных', 'Клиент, программа, прогресс и общение должны быть связаны, а не лежать по разным местам.'],
      ['Аналитика прогресса', 'Хорошая CRM для тренера показывает динамику клиента, а не только список контактов.'],
      ['Коммуникация', 'Общение рядом с тренировками экономит время и не теряет контекст.'],
      ['Стоимость и старт', 'Оцените, окупается ли CRM экономией времени; начать стоит с бесплатного старта.'],
    ],
    cta: { title: 'CRM, заточенная под тренера', text: 'NAORE хранит клиентов, тренировки, результаты и общение в одном месте.', label: 'CRM для тренера', href: '/crm' },
    related: [['Google-таблицы или CRM для персонального тренера', '/blog/excel-ili-crm'], ['Ведение клиентов', '/clients'], ['Автоматизация', '/automation']],
  },
];

function BlogArticle({ post }: { post: (typeof blogPosts)[number] }) {
  usePageMeta(post.title, post.description);
  return <div className="site-shell noise"><a href="#main" className="skip-link">Перейти к содержанию</a><Header /><main id="main">
    <article className="article container-wide">
      <span className="b-cluster">{post.cluster}</span>
      <h1>{post.h1}</h1>
